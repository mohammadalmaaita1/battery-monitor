...
export default function ElectricLoadingAnimation({ onComplete, duration = 3000 }: ElectricLoadingAnimationProps) {
  ...
  return (
    <div
      className={cn(
        "fixed inset-0 z-[9999] flex flex-col items-center justify-center bg-background transition-opacity duration-500 ease-out",
    ...
    >
      <div className="relative flex items-center justify-center w-64 h-64 md:w-72 md:h-72">
        <svg viewBox="0 0 200 200" xmlns="http://www.w3.org/2000/svg" className="w-full h-full text-primary overflow-visible">
          {/* Battery Body and Glow */}
          ...
          <g>
            {circuitPaths.map((path, index) => (
              <React.Fragment key={`circuit-${index}`}>
                ...
                />
                <circle 
                  cx={parseFloat(path.d.slice(path.d.lastIndexOf('L') + 1).split(' ')[0])} // Get X from last L command
                  cy={parseFloat(path.d.slice(path.d.lastIndexOf('L') + 1).split(' ')[1])} // Get Y from last L command
                  r="2.5" 
                  fill="currentColor" 
      ...
      <p className="mt-6 text-lg md:text-xl text-primary animate-fadeInOutSlow" role="status">
